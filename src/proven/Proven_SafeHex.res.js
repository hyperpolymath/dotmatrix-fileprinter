...
function decode(hexStr) {
  ...
  let bytes = Core__Array.make(numBytes, 0);
  let valid = true;
...
function padToByteLength(hexStr, targetByteLength) {
  ...
  let padding = Core__Array.make(targetByteLength - currentLength | 0, 0);
  return encode(padding.concat(bytes));
}
...
function xorHex(hexA, hexB) {
  ...
  if (bytesA.length !== bytesB.length) {
    ...
    return encode(bytesA.map((byteA, i) => {
      let byteB = bytesB[i];
      return byteA ^ byteB;
    }));
  }
