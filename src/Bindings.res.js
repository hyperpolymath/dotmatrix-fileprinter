...
function checkGforth() {
  return RuntimeBridge.invoke("check_gforth", undefined);
}

function previewStrike(bytes) {
  return RuntimeBridge.invoke("preview_forth_strike", {
    bytes: bytes
  });
}
...
function executeStrike(bytes, path) {
  if (Proven_SafePath.isSafe(path)) {
    return RuntimeBridge.invoke("execute_forth_strike", {
      bytes: bytes,
      path: path
    });
  } else {
...
function verifySubstrate(path) {
  if (Proven_SafePath.isSafe(path)) {
    return RuntimeBridge.invoke("verify_substrate", {
      path: path
    });
  } else {
    return rejectWithMessage("Invalid path: contains traversal sequences");
